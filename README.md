...
## Teste de performance (k6) — Smoke test
...
Foi executado um teste simples de carga com k6:

- 5 usuários virtuais
- duração: 10 segundos
- endpoint testado: `POST /orders`

Resultados principais:

- ~50 requisições enviadas
- tempo médio de resposta: ~2ms
- p95: ~6ms
- taxa de falha: ~20%

Observação:
Durante o teste, aproximadamente 20% das requisições falharam devido a colisões de orderId sob concorrência, indicando um possível risco na estratégia de geração de identificadores em cenários de carga.

Script utilizado: `performance/orders-load.js`
Evidência: `docs/evidences/k6-run.png`
